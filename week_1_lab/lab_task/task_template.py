...
data = np.loadtxt('./week_1_lab/lab_task/simulated_temperatures.csv', delimiter=',', skiprows=1)
...
plt.plot(days, temperatures, color='blue', label='Temperature')

# Mark hot days
plt.scatter(hot_days, temperatures[temperatures > hot_threshold], color='red', label='Hot days')

# Mark cold days
plt.scatter(cold_days, temperatures[temperatures < cold_threshold], color='green', label='Cold days')

# Plot mean line
plt.axhline(y=mean_temperature, color='gray', linestyle='--', label='Mean Temperature')
